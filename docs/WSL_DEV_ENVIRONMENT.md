# ZicBox WSL Development Environment
...
```bash
cd /mnt/e/PiDev/zicBox  # or your workspace path
...
### Pi Configuration (defaults in makefile)

| Variable | Default Value |
|----------|---------------|
| `PI_TARGET` | `root@192.168.1.41` |
...
# Manual deploy of a single plugin
sshpass -p password scp \
  build/pixel_arm64/libs/audio/libzic_SynthMultiEngine.so \
  root@192.168.1.41:/opt/zicBox/libs/audio/

# Restart zicBox on Pi
sshpass -p password ssh root@192.168.1.41 "systemctl restart zicbox"
```

### Monitor Pi Output

```bash
sshpass -p password ssh root@192.168.1.41 "journalctl -u zicbox -f"
```
...
- [CONTRIBUTE.md](../CONTRIBUTE.md) – General contribution guide
- [docs/BUILDER.md](BUILDER.md) – Hardware and build system overview
- [Discord](https://discord.gg/65HTx7z9qg) – Community support
