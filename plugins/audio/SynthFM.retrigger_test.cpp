#include "plugins/audio/__deprecated/SynthFM.h"
...
int main()
{
    ...
    AudioPlugin::Config cfg = { std::string("zicLegacyFM"), synthJson, 0 };
    SynthFM synth(props, cfg);

    float buffer[16] = {0.0f};

    // Setup a very fast retrig
    ...
    std::cout << "Max per-sample delta (legacy SynthFM): " << maxDelta << std::endl;
    return 0;
}
